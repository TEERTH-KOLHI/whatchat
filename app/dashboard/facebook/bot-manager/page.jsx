...
import { Plus, Zap, MessageSquare, Trash2, Power, X } from 'lucide-react';
...
export default function FacebookBotManagerPage() {
    ...
    const fetchBots = async () => {
        setLoading(true);
        try {
            const res = await fetch('/api/facebook/bot-manager');
        ...
        try {
            const res = await fetch('/api/facebook/bot-manager', {
                ...
                body: JSON.stringify({
                    ...
                    reply: newReply || 'Hello! How can I help you?',
                    type: 'Text Trigger'
                })
            });
            if (res.ok) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await fetch(`/api/facebook/bot-manager?id=${id}`, { method: 'DELETE' });
            fetchBots();
        } catch (err) {
            console.error(err);
        }
    ...
    const handleToggleStatus = async (bot) => {
        ...
        try {
            await fetch('/api/facebook/bot-manager', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <div className="dashboard-page" style={{ paddingBottom: '3rem', position: 'relative' }}>
                ...
                <div style={{
                    ...
                    <div style={{ background: 'white', padding: '2rem', borderRadius: '8px', width: '450px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                            <h3 style={{ margin: 0 }}>Create New Bot Reply</h3>
            ...
            <div className="page-header-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <div>
                    <h1 className="page-title" style={{ margin: '0 0 0.5rem 0', fontSize: '1.5rem', fontWeight: 'bold', color: '#111827' }}>Facebook Bot Manager</h1>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '2rem' }}>
                ...
                <div style={{ background: 'white', padding: '1.5rem', borderRadius: '8px', border: '1px solid #e5e7eb', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                    <div style={{ color: '#6b7280', fontSize: '0.9rem', marginBottom: '0.5rem' }}>Total Replies Sent</div>
                                ...
                                <tr key={bot.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                                    <td style={{ padding: '1rem 1.5rem' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                                            <div style={{ width: '32px', height: '32px', borderRadius: '6px', background: '#eff6ff', color: '#1d4ed8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                <Zap size={16} />
                                            </div>
